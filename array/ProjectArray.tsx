...
프로젝트 초기에는 각 섹션을 세로로 배치하여 한 페이지에 구성하고, 기본 스크롤 및 헤더 메뉴 클릭을 통해 섹션 간 이동이 가능하도록 설계했습니다.
그러나 전반적인 레이아웃에 상대적인 단위를 사용하면서, 섹션 위치가 달라져 헤더 메뉴 클릭 시 선택한 섹션으로 정확히 이동하지 못하는 문제가 발생했습니다.
또한, 단순히 스크롤만으로 이동하는 방식이 사용자에게 다소 단조롭다는 피드백을 받아 개선이 필요했습니다.

• 해결 과정 💡:

초기에는 window.scrollBy를 사용해 상대적인 스크롤 이동을 구현했으나, 뷰포트 크기 변화로 인해 스크롤 위치가 부정확해지는 문제가 있었습니다.
이를 scrollTo 메서드로 수정하여 섹션 위치를 절대적으로 설정함으로써 문제를 해결했습니다.
또한, 상하 이동 버튼을 추가하여 스크롤 동작을 직관적으로 변경하고, 화면 전환 방식을 카드 넘기기 방식으로 개선했습니다.
각 섹션의 주요 컴포넌트를 뷰포트 크기에 맞춰 재조정하여 자연스러운 전환 효과와 안정적인 반응형 디자인을 구현했습니다.

• 결과 & 느낀점 💡:

섹션 간 이동이 정확해지고, 버튼을 활용한 인터페이스로 사용자 경험을 개선했습니다.
다양한 화면 크기에서도 안정적으로 작동하는 반응형 디자인을 통해 사용자 피드백을 적극 반영하며, UX 개선의 중요성을 체감할 수 있었습니다.`,
    "webSite" : {
            "Demo" : "/",
