...
• 스타일링 : CSS Modules
• 인프라 : Vercel`,
        "mainFunction" : `• 전체 사이트 디자인
• 자기소개 및 참여 프로젝트 소개
• 섹션 스크롤 이동 기능
• 프로젝트 세부 내용을 담은 모달창 제작`,
        "MyContributions": `• Figma를 이용하여 데스크톱과 휴대폰에 최적화된 반응형 디자인 제작과 적용
• 브라우저 API와 이벤트 리스너를 이용해 버튼 클릭 시 부드러운 스크롤 이동과 섹션 별 URL 변경 구현
• React Hook과 브라우저 API를 이용하여 모달 창 구현`,
        "troubleShooting" : `✔️ 섹션 스크롤 이동 및 반응형 디자인 개선

• 문제 상황 💡:

처음에는 각 섹션을 한 페이지 내에 세로로 이어놓고, 헤더 메뉴에서 섹션명을 클릭하면 해당 섹션으로 스크롤되는 방식으로 구현했습니다.
하지만 브라우저 창 크기에 따라 섹션의 위치가 예상과 다르게 조정되고, 스크롤 이벤트가 과도하게 발생하면서 성능 저하가 발생했습니다.
또한, 이 단순한 스크롤 방식이 다소 단조롭다는 피드백이 있어, 사용자 반응이 좋지 않았습니다.

• 해결 과정 💡:

기존에는 window.scrollBy를 사용하여 상대적인 스크롤 이동 방식으로 구현했으나 뷰포트 크기에 따라 스크롤 위치가 달라지는 문제가 발생했습니다.
이를 해결하기 위해 scrollTo 메서드로 수정하여 절대적인 위치로 스크롤이 이동하도록 했습니다.
또한, 스크롤 이벤트를 비활성화하고 상하 이동 버튼을 추가하여 카드가 넘어가는 방식으로 화면 전환을 구현했습니다.
이로 인해 불필요한 스크롤 이벤트 발생을 방지했습니다.
이어서, 각 섹션의 주요 컴포넌트를 뷰포트 크기에 맞춰 디자인과 크기를 조정하여, 카드가 넘어가는 식으로 자연스럽게 표시되도록 했습니다.

• 결과 & 느낀점 💡:

스크롤 위치와 섹션 간 이동이 정확해지고, 버튼을 활용한 직관적인 인터페이스로 사용자 경험이 개선되었습니다.
다양한 화면 크기에서도 안정적으로 작동하는 반응형 디자인을 통해 사용자 피드백을 반영하는 과정의 중요성을 다시 한번 느낄 수 있었습니다.`,
    "webSite" : {
            "Demo" : "/",
